...
RF.runTeleopButton = function(button) {
  ...
  if (bind.action === "INTAKE") {
    ...
    RF.collectNearbyGamePieces(RF.state.pose);
    RF.log("TeleOp " + button + ": INTAKE " + seconds + "s at " + power);
  } else if (bind.action === "SHOOT") {
    const target = (bind.args[0] || "BLUE").toUpperCase();
    const power = RF.clamp(Number(bind.args[1]) || 0.9, 0.1, 1);
    const angle = RF.clamp(Number(bind.args[2]) || 42, 10, 80);
    RF.state.teleopTime += c.shooterSpinUpTime + 0.35;
    RF.simulateShot(target, power, angle, "teleop", RF.state.pose, true);
    RF.log("TeleOp " + button + ": SHOOT " + target + " at " + power + ", " + angle + "°");
  ...
  } else if (bind.action === "OPEN_GATE") {
    RF.unlockStoragePieces("teleop");
    RF.log("TeleOp " + button + ": gate opened.");
  } else if (bind.action === "SCORE_BASKET") {
    RF.scoreInventoryInBasket("teleop");
    RF.log("TeleOp " + button + ": score basket requested.");
  } else {
    RF.log("Unknown TeleOp binding action: " + bind.action, "error");
  }

  RF.checkParking("teleop");
  RF.el("teleopTimeStat").textContent = RF.round(RF.state.teleopTime, 2) + "s";
  RF.showToast("TeleOp " + button + " executed");
};


RF.teleopDriveContinuous = function(forward, strafe, turn, deltaTime) {
  const c = RF.getConfig();
  const scale = RF.clamp(Number(RF.inputs.teleopSpeedScale.value) || 0.75, 0.1, 1);
  const p = scale;
  const distance = c.maxSpeed * p * deltaTime;
  ...
  const from = { ...RF.state.pose };
  const requested = {
    x: RF.state.pose.x + dx,
    y: RF.state.pose.y + dy,
    heading: RF.normalizeAngle(RF.state.pose.heading + turn * 160 * deltaTime * p)
  };

  const resolved = RF.resolveAllowedPoseAlongSegment(from, requested);

  if (!RF.state.routePoints.length) {
    RF.state.routePoints.push({ x: RF.state.pose.x, y: RF.state.pose.y });
  }

  const last = RF.state.routePoints[RF.state.routePoints.length - 1];
  const movedEnough = Math.hypot(resolved.pose.x - last.x, resolved.pose.y - last.y) > 0.35;

  if (movedEnough) {
    RF.state.routePoints.push({
      x: RF.clamp(resolved.pose.x, 0, RF.FIELD_SIZE),
      y: RF.clamp(resolved.pose.y, 0, RF.FIELD_SIZE)
    });
    RF.drawPath(RF.state.routePoints);
  }

  RF.setRobotPose(resolved.pose, false, true);

  if (resolved.blocked) {
    if (!RF.teleopDriveContinuous.lastBlockedLog || performance.now() - RF.teleopDriveContinuous.lastBlockedLog > 700) {
      RF.log("TeleOp collision rule: field object blocked the robot.", "error");
      RF.showToast("Field object blocks robot movement");
      RF.teleopDriveContinuous.lastBlockedLog = performance.now();
    }
  }

  RF.state.teleopTime += deltaTime;
  RF.el("teleopTimeStat").textContent = RF.round(RF.state.teleopTime, 2) + "s";
  RF.checkParking("teleop");
};
...
RF.teleopDrive = function(forward, strafe, turn, seconds = 0.22, power = null) {
  // Scripted TeleOp movement is simulated by small continuous steps.
  const requestedPower = power ?? Number(RF.inputs.teleopSpeedScale.value) ?? 0.75;
  const oldScale = RF.inputs.teleopSpeedScale.value;
  RF.inputs.teleopSpeedScale.value = RF.clamp(requestedPower, 0.1, 1);

  const steps = Math.max(1, Math.ceil(seconds / 0.025));
  const dt = seconds / steps;

  for (let i = 0; i < steps; i++) {
    RF.teleopDriveContinuous(forward, strafe, turn, dt);
  }

  RF.inputs.teleopSpeedScale.value = oldScale;
  RF.log("TeleOp drive [" + RF.inputs.controlMode.value + "-centric]: f=" + forward + " s=" + strafe + " turn=" + turn + " for " + RF.round(seconds, 2) + "s.");
};
